...
// LOAD RANKS AND PAY POINTS
function loadRanks() {
  rank.innerHTML = '';
  const ranks = Object.keys(PAY[force.value]);
  ranks.forEach(r => rank.add(new Option(r,r)));
  loadPoints();
}

function loadPoints() {
  point.innerHTML = '';
  const points = Object.keys(PAY[force.value][rank.value] || {});
  points.forEach(p => point.add(new Option(p,p)));
  calculate();
}
...
saveBtn.onclick = () => {
  ...
  if(!rank.value || !point.value) return alert('Please select rank and pay point');
...
function renderLog(){
  ...
  getEntries().forEach((e,i) => {
    let entryHTML = `<div class="entry">${e.date} • ${e.rank} PP${e.point} • ${e.hours}h • £${e.net.toFixed(2)}`;
    if(e.receipt) entryHTML += ` <img src="${e.receipt}" alt="Receipt">`;
    entryHTML += '</div>';
...
exportCal.onclick = () => {
  ...
  entries.forEach((e,i) => {
    ...
    ics += `BEGIN:VEVENT\r\nUID:police-ot-${i}-${d}@local\r\nDTSTAMP:${now}Z\r\nDTSTART;VALUE=DATE:${d.slice(0,8)}\r\nSUMMARY:Police OT ${e.hours}h\r\nEND:VEVENT\r\n`;
  });
